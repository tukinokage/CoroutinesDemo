...
public class StackLayoutManager extends RecyclerView.LayoutManager {
    ...
    private float heightViewGap = 150;
    ...
    private float getMinOffset() {
        ...
        if(minOffer != -1){ return minOffer; }
        return minOffer = (getWidth() - childWidth) / 2;
    }
    ...
    private int fillHorizontalLeft(RecyclerView.Recycler recycler, RecyclerView.State state, int dx) {
        ...
        startX = normalViewGap + getMinOffset();
        ...
        int maxViewY = (int) (getHeight() - childHeight);
        focusY = maxViewY;
        int mViewY = (int) ( (maxViewY - heightViewGap + heightViewGap * fraction)) ;
        /**子view距离底部消失的距离**/
        float fromBottomDistance = 2 * heightViewGap;
        /** 小于该范围的y不可见，用于透明度变化*/
        float disVisFormBottomY = getHeight() - fromBottomDistance;
        ...
        for (int i = mFirstVisiPos; i <= mLastVisiPos; i++) {
             ...
             * */
            final float fractionScale =  (1f-minScale) * b / getHeight() + minScale;
            ...
            float alphaOffer = 1f;
            if(startX < getMinOffset()){
                alphaOffer = startX / getMinOffset();
            }else if(b < disVisFormBottomY){
                alphaOffer = 0f;
            } else if(b < getHeight()){
                alphaOffer = (b - disVisFormBottomY) *1.0f / fromBottomDistance;
            }

            item.setAlpha(alphaOffer);

            layoutDecoratedWithMargins(item, l, t, r, b);

            //第二个开始保持x为getMinOffset()
            startX = getMinOffset();
            if(i != mFirstVisiPos){
                mViewY -= heightViewGap;
            }
            if (mViewY > getHeight() - getPaddingTop()) {
                mLastVisiPos = i;
                break;
            }
        }
